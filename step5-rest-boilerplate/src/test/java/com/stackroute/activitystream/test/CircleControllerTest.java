...
public class CircleControllerTest {
	...
	public void setup() {
		...
		circleMockMvc = MockMvcBuilders.standaloneSetup(circleController).build();
	...
	public void testCreateCircle() throws Exception {
		...
		circleMockMvc
				.perform(post("/api/circle").contentType(MediaType.APPLICATION_JSON).content(asJsonString(newCircle))
						.session((MockHttpSession) session))
				.andExpect(status().isCreated()).andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
				.andExpect(jsonPath("$.circleName", is("Spring"))).andExpect(jsonPath("$.creatorId", is("john")));
	...
	public void testCreateDuplicateCircleFailure() throws Exception {
		...
		circleMockMvc.perform(post("/api/circle").contentType(MediaType.APPLICATION_JSON)
				.content(asJsonString(newCircle)).session((MockHttpSession) session)).andExpect(status().isConflict());
	...
	public void testCreateCircleFailure() throws Exception {
		...
		circleMockMvc.perform(post("/api/circle").contentType(MediaType.APPLICATION_JSON)
				.content(asJsonString(newCircle)).session((MockHttpSession) session))
	...
	public void testGetAllCircles() throws Exception {
		...
		List<Circle> circles = Arrays.asList(new Circle("Angular", "john", new Timestamp(System.currentTimeMillis())),
				new Circle("Spring", "chris", new Timestamp(System.currentTimeMillis())));

		when(circleService.getAllCircles()).thenReturn(circles);

		circleMockMvc.perform(get("/api/circle").session((MockHttpSession) session)).andExpect(status().isOk())
				.andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
				.andExpect(jsonPath("$", hasSize(2))).andExpect(jsonPath("$[0].circleName", is("Angular")))
				.andExpect(jsonPath("$[0].creatorId", is("john"))).andExpect(jsonPath("$[1].circleName", is("Spring")))
	...
	public void testGetAllCirclesFailure() throws Exception {
		...
		circleMockMvc.perform(get("/api/circle").session((MockHttpSession) session))
				.andExpect(status().isUnauthorized());
	...
	public void testGetAllMatchingCircles() throws Exception {
		...
		List<Circle> circles = Arrays.asList(new Circle("Spring", "chris", new Timestamp(System.currentTimeMillis())),
		...
		circleMockMvc.perform(get("/api/circle/search/spring").session((MockHttpSession) session))
				.andExpect(status().isOk()).andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
				.andExpect(jsonPath("$", hasSize(2))).andExpect(jsonPath("$[0].circleName", is("Spring")))
